...
CREATE INDEX IF NOT EXISTS idx_utilisateur_role ON utilisateurs(role_id);
CREATE INDEX IF NOT EXISTS idx_vente_client ON ventes(client_id);
CREATE INDEX IF NOT EXISTS idx_vente_utilisateur ON ventes(utilisateur_id);
...
CREATE INDEX IF NOT EXISTS idx_dette_vente ON dettes(vente_id);
CREATE INDEX IF NOT EXISTS idx_dette_client ON dettes(client_id);
CREATE INDEX IF NOT EXISTS idx_paiement_dette ON paiements(dette_id);
CREATE INDEX IF NOT EXISTS idx_paiement_mode ON paiements(mode_paiement_id);
CREATE INDEX IF NOT EXISTS idx_appro_fournisseur ON approvisionnements(fournisseur_id);
CREATE INDEX IF NOT EXISTS idx_ligne_appro_appro ON lignes_approvisionnement(approvisionnement_id);
CREATE INDEX IF NOT EXISTS idx_ligne_appro_produit ON lignes_approvisionnement(produit_id);
...
CREATE INDEX IF NOT EXISTS idx_dettes_ref ON dettes(ref);
CREATE INDEX IF NOT EXISTS idx_appro_reference_bl ON approvisionnements(reference_bl);
...
INSERT OR IGNORE INTO modes_paiement (nom, description) VALUES
    ('Wave', 'Paiement mobile via Wave'),
    ('Orange Money', 'Paiement mobile via Orange Money'),
    ('Especes', 'Paiement en espèces'),
    ('Virement', 'Virement bancaire');

INSERT OR IGNORE INTO utilisateurs (nom, prenom, email, password, telephone, adresse, role_id)
...
CREATE VIEW IF NOT EXISTS v_dettes_clients AS
SELECT 
    c.id AS client_id,
...
CREATE VIEW IF NOT EXISTS v_stock_produits AS
SELECT 
    p.id,
    p.code,
    p.libelle,
